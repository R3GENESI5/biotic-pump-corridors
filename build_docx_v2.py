...
add_figure(OUTPUT_DIR / "fig1_corridor_map.png",
           "Figure 1. Study area showing the two tropical moisture corridors. "
           "The Amazon corridor (green) runs from Belem to Manaus (~1,300 km). "
           "The Congo corridor (yellow) runs from Douala to Kisangani (~1,700 km). "
           "Shaded bands indicate the latitude averaging domains. "
           "Dotted rectangle shows the wider Eastern Amazon domain used for spatial correlation.")

# Figure 2: Causal chain
add_figure(OUTPUT_DIR / "fig_causal_chain_v3.png",
           "Figure 2. Conceptual framework: the causal chain of biotic pump breakdown "
           "with the satellite instruments and key findings mapped to each step. "
           "The dashed line separates results presented in this study (above) from "
           "future work using ERA5 reanalysis (below). The feedback arrow indicates "
           "the self-reinforcing cycle predicted by theory.")
...
doc.add_paragraph(
    "Along each corridor, we define three segments for gradient analysis (Table S1). "
    "The coast-to-interior gradient is computed as the area-weighted mean of the "
    "interior segment minus the coast segment."
)
...
doc.add_paragraph(
    ...
    "evapotranspiration at 500 m resolution in the MODIS sinusoidal tile projection. "
    "Tiles covering both corridors were downloaded for 2019 and 2023, reprojected from "
    "sinusoidal to geographic coordinates using standard MODIS grid parameters (Earth "
    "radius 6,371,007 m, tile size 1,111,951 m), and aggregated to 1-degree resolution "
    "using 2D binned statistics. Fill values (65535) and unrealistic values (>3,000 mm/yr "
    "raw) were masked; raw values were scaled by 0.1 to convert to mm/yr."
)
doc.add_paragraph(
    "MODIS MOD13C2 Version 061 provides monthly NDVI at 0.05-degree global resolution "
    "(Climate Modeling Grid). All monthly composites for 2019 (12 files) and 2023 (11 "
    "files) were averaged annually. Grid cells with NDVI > 0.6 in 2019 were classified "
    "as dense forest. For comparison with 1-degree data, NDVI was block-averaged (20 "
    "\u00d7 20 pixels)."
)
doc.add_paragraph(
    "AIRS L3 Version 7.0 (AIRS3STM; Tian et al., 2020) provides total column water "
    "vapour (ascending orbit, TotH2OVap_A) at 1\u00b0 \u00d7 1\u00b0 monthly resolution. All months "
    "for 2019 and 2023 were downloaded and averaged annually."
)
doc.add_paragraph(
    "The Oceanic Ni\u00f1o Index (ONI) was obtained from the NOAA Climate Prediction Center. "
    "Annual mean ONI was computed for regression analysis, and years were classified by "
    "peak absolute ONI within the calendar year: El Ni\u00f1o (\u22650.5), La Ni\u00f1a (\u2264\u22120.5), "
    "or neutral."
)
...
add_figure(OUTPUT_DIR / "wide_amazon_olr.png",
           "Figure 3. Eastern Amazon OLR: 2019 (left), 2023 (centre), "
           "difference (right). Dashed line shows the Belem-Manaus corridor.")

add_figure(OUTPUT_DIR / "et_comparison_maraba.png",
           "Figure 4. Maraba region evapotranspiration (MODIS MOD16A3GF): "
...
add_figure(OUTPUT_DIR / "corridor_seasonal_transects.png",
           "Figure 8. Seasonal decomposition of corridor changes (2023 minus 2019). "
           "SON (yellow) dominates the signal at the interior end.")

add_figure(OUTPUT_DIR / "corridor_seasonal_bars.png",
           "Figure 9. Seasonal breakdown by corridor segment. "
...
add_figure(OUTPUT_DIR / "corridor_gradient_evolution.png",
           "Figure 10. Coast-to-interior gradient evolution along the Amazon corridor, "
           "2001-2025. Dashed lines show linear trends.")

add_figure(OUTPUT_DIR / "corridor_SON_multiyear.png",
           "Figure 11. SON-only time series along the Amazon corridor, 2001-2025.")
...
add_figure(OUTPUT_DIR / "enso_adjusted_trends.png",
           "Figure 12. ENSO-adjusted corridor gradient trends. After removing the ONI "
           "signal, the degradation trend persists (p = 0.001 for cloud fraction).")

add_figure(OUTPUT_DIR / "enso_2023_vs_other_elnino.png",
           "Figure 13. 2023 compared to other El Nino years. "
...
add_figure(OUTPUT_DIR / "ndvi_forest_vs_nonforest.png",
           "Figure 14. NDVI change in intact forest vs non-forest cells. "
           "88% of dense forest cells show NDVI decline.")

add_figure(OUTPUT_DIR / "ndvi_vs_et_scatter.png",
           "Figure 15. NDVI change vs ET change across the Eastern Amazon.")
...
add_figure(OUTPUT_DIR / "wide_amazon_scatter.png",
           "Figure 16. ET vs atmospheric response per grid cell in the Eastern Amazon.")
...
add_heading("4.4 Basin-wide trends versus corridor-specific signals", level=2)
doc.add_paragraph(
    "An important nuance arises when comparing basin-wide and corridor-specific ET "
    "trends. MODIS-derived ET anomalies for the Amazon basin as a whole show an "
    "increasing trend over the past decade, with land surface temperature also rising "
    "and precipitation anomalies showing high variability without a clear directional "
    "trend. At first glance, increasing basin-wide ET appears to contradict our finding "
    "of ET decline along the Belem\u2013Manaus corridor."
)
doc.add_paragraph(
    "However, the basin-wide average masks critical spatial heterogeneity. The eastern "
    "Amazon corridor \u2014 where the biotic pump must relay moisture from coast to interior "
    "\u2014 shows a different trajectory from the western Amazon, which receives moisture from "
    "multiple pathways and has experienced less deforestation. Our corridor analysis "
    "reveals that ET declined by 66 mm/yr (\u22124.5%) in the Marab\u00e1 subregion, with 88% of "
    "intact forest cells showing NDVI decline, while the wider basin partially compensated. "
    "This spatial disaggregation is essential: the biotic pump is a corridor-scale mechanism, "
    "and its failure manifests along specific transport paths, not as a basin-wide uniform "
    "signal. Basin-averaged metrics may remain stable or even increase while the critical "
    "moisture relay is degrading."
)

add_heading("4.5 Amazon and Congo: different stages of degradation", level=2)
doc.add_paragraph(
...
add_heading("4.6 Moisture displacement and extreme weather", level=2)
doc.add_paragraph(
    "During September 2023, CERES OLR showed positive anomalies of +12.5 W/m\u00b2 over "
    "the Amazon and +5.0 W/m\u00b2 over the Congo (relative to the 2001\u20132018 September "
    "climatology), while the eastern Mediterranean showed negative OLR anomalies "
    "(\u22124.6 W/m\u00b2) coinciding with Storm Daniel, which caused catastrophic flooding "
    "in Libya. A similar spatial dipole appeared in September 2024, with tropical "
    "positive anomalies and negative anomalies over Central Europe coinciding with "
    "Storm Boris (Supplementary Figures S1\u2013S4)."
)
doc.add_paragraph(
    "This spatial pattern \u2014 OLR surplus over tropical forests, deficit at extratropical "
    "storm locations \u2014 is consistent with the hypothesis that moisture displaced from "
    "weakened tropical pumps was rerouted into mid-latitude weather systems. However, "
    "the atmospheric dynamics linking tropical moisture sources to extratropical extreme "
    "precipitation involve complex teleconnections and jet stream interactions that cannot "
    "be resolved from monthly satellite data. Moisture source attribution through "
    "back-trajectory analysis (e.g., HYSPLIT; Stein et al., 2015) and comparison with "
    "ERA5 reanalysis moisture transport fields would be needed to test this hypothesis, "
    "and we identify this as a priority for future investigation."
)

add_heading("4.7 Limitations", level=2)
doc.add_paragraph(
...
add_figure(OUTPUT_DIR / "final_summary.png",
           "Figure 17. Summary figure: (a) OLR difference 2023\u20132019 with corridor paths "
           "and storm locations, (b) Amazon corridor transect with OLR and water vapour, "
           "(c) Congo corridor transect, (d) seasonal decomposition showing SON dominance, "
           "(e) ENSO-adjusted cloud fraction gradient trend (p = 0.001), (f) NDVI change "
           "in intact forest, (g) ET vs OLR spatial correlation.")
...
doc.add_paragraph(
    "The following supplementary figures are available: "
    "S1\u2013S2: Storm Daniel September 2023 OLR and cloud anomaly maps with forest "
    "source regions and storm location; "
    "S3\u2013S4: Storm Boris September 2024 OLR and cloud anomaly maps; "
    "S5: AIRS water vapour anomaly map for September 2023; "
    "S6: Moisture dipole quantification bar chart; "
    "S7\u2013S8: Full ENSO control analysis (all variables, both corridors); "
    "S9: Multi-year time series for all corridor variables; "
    "S10\u2013S11: Congo corridor gradient evolution and SON time series; "
    "S12\u2013S14: NDVI and ET comparison maps for both regions; "
    "S15: ET vs OLR scatter for the Maraba subregion."
)
...
# SAVE
# =========================================================================
output_path = PAPER_DIR / "Shahid_2025_Biotic_Pump_Corridors_final.docx"
